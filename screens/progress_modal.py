...
class ProgressModal(ModalScreen):
    ...
    def __init__(self, instance: InstanceConfig, dependencies: list[dict] | None = None, mode: str = 'modpack'):
        super().__init__()
    ...
    async def start_install(self):
        try:
            if self.mode == 'modpack' and self.dependencies:
                ...
                status, message = await install_modpack(self.instance, self.steps, dependencies, self.progress_bar_callback, self.step_callback, self.cancel_event)
            elif self.mode == 'modloader':
                status, message = await install_modloader(self.instance, self.modloader_steps, self.progress_bar_callback, self.step_callback, self.cancel_event)
            else:
                return
