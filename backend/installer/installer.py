...
async def install_modpack(instance: InstanceConfig, steps: list[str], dependencies: list[dict], progress_bar_callback, step_callback, cancel_event: asyncio.Event) -> tuple[int, str]:
    ...
    result = await run_fabric_installer(Path("instances") / instance.instance_id, installer_jar, mc_version, loader_version) # idk, can't test cause no java
    # !! disabled cause testing with no java !!
    # if result != 0:
    #     return 3, str(result)
    ...
    project_ids = [dep["project_id"] for dep in dependencies]
    ...
    mods_and_datapacks = await combine_project_and_version_info(projects, versions)
    mods = [e for e in mods_and_datapacks if e["type"] == "mod"]
    datapacks = [e for e in mods_and_datapacks if e["type"] == "datapack"]
    ...
    len_mods = len(mods_and_datapacks)
...
async def install_modloader(instance: InstanceConfig, steps: list[str], progress_bar_callback, step_callback, cancel_event: asyncio.Event) -> tuple[int, str]:
    async def smooth_step_callback(step: str, label_id: int=1):
        step_callback(step, label_id)
        await asyncio.sleep(0.1)
